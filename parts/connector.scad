...
module connector_female() {
    connector_shape(plug_height_female, plug_female_offset);
}
...
module guide_shape(connector_h, offset=0, bonus_xy=0, bonus_h=0) {
    x = plug_guide_diameter + offset*2 + bonus_xy*2;
    y = plug_guide_width + offset*2 + bonus_xy*2;
...
module main_cone_inner(h, offset=0) {
    inner_dmax = plug_diameter_max + offset*2 - plug_wall_depth*2;
    inner_dmin = plug_diameter_min + offset*2 - plug_wall_depth*2;
